...
namespace State.PlayerState
{
    public class RunningState : IPlayerState
    {
        ...
        public RunningState(PlayerMVCFacade facade, IStateController stateController,
        ...
        {
            _facade.PlayAnimation("Run", 0.1f);
        }
        ...
        public async UniTask Tick()
        {
            ...
            if (!_runGetter())
            {
                _stateController.ChangeState(StateKey.Walk);
                return;
            }
            _facade.ApplyPlanarSpeed(mv.normalized, _facade.RunSpeed);
            if (_attackGetter())
            {
